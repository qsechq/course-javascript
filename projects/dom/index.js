...
function findAllPSiblings(where) {
  ...
  for (const args of where.children) {
    if (args.nodeName === 'P') {
      arr.push(args.previousElementSibling);
    }
  }
...
function deleteTextNodes(where) {
  for (const rm of where.childNodes) {
    rm.remove(rm.textContent);
  }
  return where;
}
...
function deleteTextNodesRecursive(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    const rm = where.childNodes[i];

    rm.nodeType === 3 ? where.removeChild(rm) : deleteTextNodesRecursive(rm);
    rm.nodeType === 3 ? i-- : 0;
  }
  return where;
}
